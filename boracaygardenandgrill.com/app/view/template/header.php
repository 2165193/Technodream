<?php $this->suspensionRedirect($view); ?>
<!DOCTYPE html>
<html lang="en" <?php $this->helpers->htmlClasses(); ?>>
<head>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0"/>
	<meta name="apple-mobile-web-app-capable" content="yes" />
	<meta name="apple-mobile-web-app-status-bar-style" content="black" />

	<?php $this->helpers->seo($view); ?>
	<link rel="icon" href="public/images/favicon.png" type="image/x-icon">
	<!-- <link rel="stylesheet" href="//maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous"> -->
	<link href="<?php echo URL; ?>public/styles/style.css" rel="stylesheet">
	<link rel="stylesheet" href="//cdnjs.cloudflare.com/ajax/libs/cookieconsent2/3.0.3/cookieconsent.min.css" />
	<link rel="stylesheet" href="<?php echo URL; ?>public/fancybox/source/jquery.fancybox.css" media="screen" />
	<?php $this->helpers->analytics(); ?>
</head>

<body <?php $this->helpers->bodyClasses($view); ?>>
<?php $this->checkSuspensionHeader(); ?>
	<header>
		<div id="header">
			<div class="row">
				<nav>
					<a href="#" id="pull"><strong>MENU</strong></a>
					<ul>
						<li <?php $this->helpers->isActiveMenu("home"); ?>><a href="<?php echo URL ?>">HOME</a></li>
						<li <?php $this->helpers->isActiveMenu("party"); ?>><a href="<?php echo URL ?>party#content">PARTY TRAYS</a></li>
						<li <?php $this->helpers->isActiveMenu("menu"); ?>><a href="<?php echo URL ?>menu#content">OUR MENU</a></li>
						<li <?php $this->helpers->isActiveMenu("gallery"); ?>><a href="<?php echo URL ?>gallery#content">GALLERY</a></li>
						<li <?php $this->helpers->isActiveMenu("reviews"); ?>><a href="<?php echo URL ?>reviews#content">REVIEWS</a></li>
						<li <?php $this->helpers->isActiveMenu("directions"); ?>><a href="<?php echo URL ?>directions#content">DIRECTIONS</a></li>
						<li <?php $this->helpers->isActiveMenu("contact"); ?>><a href="<?php echo URL ?>contact#content">CONTACT US</a></li>
					</ul>
				</nav>
			</div>		
		</div>
	</header>

	<div id="banner">
		<div class="row">
			<a href="<?php echo URL ?>"><img src="./public/images/common/mainLogo.png" alt="Main Logo" class="mainLogo"></a>
			<div class="row-1">
				<div class="container">
					<span>We serve guests for both lunch and dinner with a large menu selection filled with</span>
					<h4>Unique Filipino & Asian Dishes</h4>
				</div>
			</div>
			<div class="row-2">
				<div class="col-1">
					<div class="container">
						<dl>
							<dt><img src="public/images/common/bannerImg1.png" alt=""></dt>
							<dd><?php $this->info("address");?></dd>
						</dl>
						<dl>
							<dt><img src="public/images/common/bannerImg2.png" alt=""></dt>
							<dd><?php $this->info(["phone","tel"]);?></dd>
						</dl>
						<dl>
							<dt><img src="public/images/common/bannerImg3.png" alt=""></dt>
							<dd><?php $this->info(["phone2","tel"]);?></dd>
						</dl>
					</div>
				</div>
				<div class="col-2">
					<div class="cols cont-1">
						<span>FOLLOW US</span>
						<p>
							<img src="public/images/common/contImg1.png" alt="">
							<img src="public/images/common/contImg2.png" alt="">
						</p>
					</div>
					<div class="cols cont-2">
						<span>ALSO VISIT</span>
						<p>Boracay Garden</p>
					</div>
				</div>
			</div>
		</div>
	</div>